...
- [Usage](#usage)
    - [Issuing](#issuing)
        ...
        - [BillingTransactions](#query-issuingbillingtransactions): View the transactions generated by your Issuing billing invoices
    - [Pix](#pix)
...
```javascript
await (async() => {
    let billingInvoice = await starkinfra.issuingBillingInvoice.get('9302937674764487');
...
You can query multiples available Billing Transactions using this resource.

```javascript
await (async() => {
    let billingTransactions = await starkinfra.issuingBillingTransaction.query({
        after: '2023-01-01',
        before: '2024-03-01',
        limit: 10
    });

    for await (let billingTransaction of billingTransactions) {
...
(async() => {
    let note = await starkinfra.creditNote.cancel('5155165527080960');
    console.log(note);
})();
```
...
To retrieve the CCB disbursement pdf file, use the `starkinfra.creditNote.pdf` method with a valid (signed) CreditNote id.
...
(async() => {
    let pdf = await starkinfra.creditNote.pdf('5155165527080960');
    fs.writeFileSync('credit_note_receipt.pdf', pdf);
})();
```

### Retrieve CCB disbursement payment pdf file

To retrieve the CCB disbursement payment pdf file, use the `starkinfra.creditNote.payment` method with a valid (signed) CreditNote id.
...
(async() => {
    let pdf = await starkinfra.creditNote.payment('5155165527080960');
    fs.writeFileSync('credit_note_payment_receipt.pdf', pdf);
})();
```
