...
describe('TestIssuingPurchaseParseRight', function() {
    this.timeout(10000);
    it('test_success', async () => {
        let authorization = await starkinfra.issuingPurchase.parse(CONTENT, VALID_SIGNATURE)
    });
});
...
describe('TestIssuingPurchaseParseWrong', function() {
    ...
    it('test_success', async () => {
        let error = false;
        try {
            let authorization = await starkinfra.issuingPurchase.parse(CONTENT, INVALID_SIGNATURE)
        } catch (e) {
            error = true;
        }
        assert(error);
...
describe('TestIssuingPurchaseParseMalformed', function() {
    ...
    it('test_success', async () => {
        let error = false;
        try {
            let authorization = await starkinfra.issuingPurchase.parse(CONTENT, 'something is definitely wrong')
        } catch (e) {
            error = true;
        }
        assert(error);
...
describe('TestIssuingPurchaseResponse', function() {
    this.timeout(10000);
    it('test_success', async () => {
        const purchase = await starkinfra.issuingPurchase.response({
            'status': 'approved'
        });
