...
function preProcess(path, method, payload, query, user, version) {
    user = user || starkinfra.user;
    language = Check.language(starkinfra.language);

    if (!user) {
        throw Error('A user is required to access our API. Check our docs: https://github.com/starkinfra/sdk-node/');
    }

    let hostname = {
        'production': 'https://api.starkinfra.com/' + version,
        'sandbox': 'https://sandbox.api.starkinfra.com/' + version
    }[user.environment];
